...
Version 0.5.0 recognizes appraisal/detail screen text and combat power (`CP`/`WP`). On an appraisal screen it measures the three visible bars and reports experimental Attack/Defense/HP values plus the total IV percentage. The compact Dynamic Island shows confirmed visual badges on the left and CP or IV percentage on the right. A first beta detector watches several frames for small, moving, low-chroma sparkle highlights around the Pokémon and shows the yellow Shiny sparkle badge only after the animated evidence is stable. A missing badge is deliberately not treated as proof that a Pokémon is not Shiny.
