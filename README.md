...
Version 0.5.1 recognizes appraisal/detail screen text and combat power (`CP`/`WP`). OCR sampling now runs at a shorter interval, while two matching identities are still required before publishing a result. On an appraisal screen it supports both observed vertical bar layouts and reports experimental Attack/Defense/HP values plus the total IV percentage. The compact Dynamic Island shows short confirmed badges on the left and only the CP number or IV percentage on the right so iOS has enough room to render it. Supported badges currently cover visually detected Shiny sparkles, protected rarity classes, explicit Dynamax and XXS/XXL text, plus a conservative PvP IV-pattern hint.

The Shiny beta detector watches several frames for small, moving, low-chroma sparkle highlights around the Pokémon and shows the yellow Shiny sparkle badge only after the animated evidence is stable. It now also samples the upper central background for smaller animations such as Hoothoot's. A missing badge is deliberately not treated as proof that a Pokémon is not Shiny.

An offline catalog matches German and English species names, including player-added numeric IV annotations, and marks Legendary, Mythical, and Ultra Beast classes. Species-level Shiny availability is retained as catalog data but is no longer presented as if it described the individual Pokémon in the Dynamic Island. Event/costume catalog hints remain manual-review information until a visual form detector is calibrated. The app deliberately never marks a Pokémon as safe to transfer. Name recognition excludes the Dynamic Island area so an earlier PokeAssist result cannot be read back as the current Pokémon.
...
9. On a normal detail screen, watch for a yellow sparkle badge only when an animated Shiny background is positively detected. On an appraisal screen, the compact metric switches to IV percentage.
10. A missing Shiny badge is an unknown result, not a safe-to-transfer decision. Costume state must still be checked in Pokémon GO.
