...
Version 0.5.2 recognizes appraisal/detail screen text and combat power (`CP`/`WP`). OCR sampling runs at a short interval, while two matching identities are still required before publishing a result. On an appraisal screen it supports both observed vertical bar layouts and reports experimental Attack/Defense/HP values plus the total IV percentage. The compact Dynamic Island shows short confirmed badges on the left and only the CP number or IV percentage on the right so iOS has enough room to render it. Supported badges cover calibrated Shiny and event-form matches, protected rarity classes, explicit Dynamax and XXS/XXL text, plus a conservative PvP IV-pattern hint.
