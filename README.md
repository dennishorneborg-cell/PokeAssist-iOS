...
Version 0.5.19 avoids sending frame-counter-only updates to ActivityKit, reducing unnecessary Live Activity refreshes.

Version 0.5.18 adds Live Activity update-queue wait and ActivityKit request timing diagnostics. Request timings reflect the API call, not the system's final Dynamic Island render time. Version 0.5.17 adds capture-source diagnostics: ScreenCaptureKit callback and valid-frame rates, maximum presentation-timestamp gap, and invalid/missing frame and timestamp counts. Version 0.5.16 adds the elapsed time from the first recognized candidate to its two-frame confirmation, the age of a currently pending candidate, and how often candidate recognition restarts. Version 0.5.15 adds on-device runtime diagnostics for capture duration, thermal state, app memory, incoming/processed frame rates, and Vision/appearance analysis times. Metrics are sampled every 10 seconds, kept in bounded scalar state, and are not uploaded. Version 0.5.14 clears Shiny and event detections after two contrary visual samples and keeps the latest CP per species for the current capture, labeling reused CP as cached. Version 0.5.13 restores the previous Live Activity refresh cadence while retaining the 4-frames-per-second analysis limit. Version 0.5.12 limits frame processing to 4 frames per second to reduce sustained analysis load. iOS doesn't expose ScreenCaptureKit's frame-interval setting. Version 0.5.11 increases Dynamic Island trait-symbol sizes according to the number of detected traits, keeping common combinations more legible. Version 0.5.10 shows CP and IV together in the expanded Dynamic Island. If the open Island covers Pokémon GO's CP label, PokeAssist retains the last CP read for the same species and marks it "zuletzt erkannt". This value may belong to a previous individual of the same species, so recheck it in Pokémon GO before relying on it. IV bar analysis now reuses its Core Image context instead of recreating it on every appraisal frame.

PokeAssist is an iOS 27 technology prototype for testing full-display capture while Pokemon GO is frontmost. It uses ScreenCaptureKit to count captured frames in local diagnostics, shows recognized Pokémon information in a Live Activity / Dynamic Island, and performs an initial on-device Vision OCR pass for Pokemon detail and appraisal screens.

Version 0.5.9 recognizes appraisal/detail screen text and combat power (`CP`/`WP`). Long capture sessions use a bounded latest-frame pipeline: the app accepts at most 12 captured frames per second, at most one frame may wait for the MainActor, and stale callbacks are dropped instead of accumulating. Vision work runs inside explicit autorelease pools, while the appearance detector reuses one Core Image context. These changes prevent recognition latency and memory pressure from growing over time. OCR sampling runs at a short interval, while two matching identities are still required before publishing a result. Name recognition checks multiple Vision candidates, tolerates the common `O`/`0` confusion in names such as Ho-Oh, and searches a wider name/resource-label band. Player-added trailing number annotations are removed before species matching, including plain, superscript, subscript, parenthesized, circled, double-circled, and filled-circled digits. If those annotations replace the final one or two letters because of Pokémon GO's nickname limit, a six-character-or-longer prefix is accepted only when it resolves to exactly one catalog species. Resource labels also accept a leading candy count, allowing text such as `41 Reißlaus-Bonbon` to recover the full species safely. On an appraisal screen it supports both observed vertical bar layouts and reports experimental Attack/Defense/HP values plus the total IV percentage. The Dynamic Island packs the always-visible green capture status together with every confirmed trait into one micro badge strip. This also works in iOS's minimal Live Activity presentation while the screen-recording activity occupies the second Island slot. Supported badges cover Shiny, event costumes, protected rarity classes, explicit Dynamax and XXS/XXL text, plus a conservative PvP IV-pattern hint. Short OCR gaps no longer clear confirmed appearance badges, which prevents the compact display from alternating between green and the detected traits.
